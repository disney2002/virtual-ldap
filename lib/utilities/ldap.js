...
function makePersonEntry(dn, attrs) {
  ...
  const loginUid = attrs['pinyin'] + attrs['mobileTelephoneNumber'].slice(-4)
  // password: use pinyin + mobile first 4 numbers as password
  const userPassword = storeUserPassword(
    attrs['pinyin'] + attrs['mobileTelephoneNumber'].substr(0, 4))
