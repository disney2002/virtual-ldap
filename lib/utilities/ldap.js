...
function makePersonEntry(dn, attrs) {
  ...
  attrs.uid = attrs.pinyin + attrs.mobileTelephoneNumber.slice(-4)
  attrs.userPassword = attrs.pinyin + attrs.mobileTelephoneNumber(0, 4)
  const generatedDN = makeOrganizationUnitEntryDN(dn);
  const entry = {
    dn: generatedDN,
    attributes: Object.assign({
      objectclass: ['inetOrgPerson', 'organizationalPerson', 'person', 'top'],
      userPassword: Math.random().toString(36).substring(2, 15),
      // otpSecret: 'abcd',
      memberOf: [],
      entryDN: dn,
    }, attrs)
