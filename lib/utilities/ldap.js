...
function makePersonEntry(dn, attrs) {
  ...
  const loginName = attrs['pinyin'].replace(/[^0-9a-z]/gi, '').toLowerCase()
  const mobile = attrs['mobileTelephoneNumber']

  // uid: use pinyin + mobile last 4 numbers as unique uid
  const loginUid = loginName + mobile.slice(-4)
  console.log("login uid:", loginUid + " " + new Date().toLocaleString());
  attrs['uid'] = loginUid

  // password: use pinyin + mobile first 4 numbers as password
  const userPassword = storeUserPassword(loginName + mobile.substr(0, 4))
