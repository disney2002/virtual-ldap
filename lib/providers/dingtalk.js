...
async function fetchAllUsers(departments) {
  ...
  if (!allUsers && departments.length > 0) {
    ...
    allUsers = allUsers.filter(u=> {return (u.email || u.orgEmail)});
...
async function reloadFromDingtalkServer() {
  ...
  allLDAPUsers = allUsers.filter(u => {
    ...
    if (!(u.email || u.orgEmail)) {
      log.warn('Incorrect user missing email', u);
      return false;
    }
    return true;
  }).map(u => {
    const mail = (u.email || u.orgEmail).toLowerCase();
    ...
    const personEntry = makePersonEntry(dn, {
      ...
      pinyin: py,
      sAMAccountName: mail,
      cn: mail.substring(0, mail.indexOf('@')),
      givenName,
      sn,
      mail,
      avatarurl: u.avatar,
    ...
    log.debug('Sync person entry:', personEntry)
